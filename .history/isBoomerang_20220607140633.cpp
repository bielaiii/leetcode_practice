...
bool isBoomerang(vector<vector<int>>& points) {
      set<vector<int>> set_;
      set_.insert(points[0]);
      if(set_.count(points[1])) return 0;
      set_.insert(points[1]);
      if(set_.count(points[2])) return 0;
      if(point[0][0] == points[1][0] && points[0][0] == points[2][0] ){
            return 0;
      }else if(point[0][0] != points[1][0] && points[0][0] != points[2][0]){
