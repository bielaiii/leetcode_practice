...
void traversal_pruneTree(TreeNode * root){
    ...
    if(root->val == 0){
        TreeNode * temp_left = recur_pruneTree(roo->left);
        TreeNode * temp_right = recur_pruneTree(roo->right);
        if(temp_left == nullptr) temp->left = nullptr;
        if(temp_right == nullptr) temp->right = nullptr;
