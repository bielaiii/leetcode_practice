...
bool check_go_over(int num1, int num2, vector<int> fail_vec){
    int sum = num1 + num2;
...
bool check_duplicate(vector<int> add, vector<int> ret){
...
vector<vector<int> > threeSum(vector<int>& nums){
    ...
    while(old_i < len - 2){
        ...
        if(!check_go_over(nums[i], nums[j], fail_vec)){
            ...
            //    cout << "duplicate "<<(!check_duplicate(temp)) <<endl;
                if(!check_duplicate(temp, ret)) ret.push_back(temp);
                temp.clear();
