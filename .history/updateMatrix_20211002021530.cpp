...
vector<vector<int>> updateMatrix(vector<vector<int>>& mat) {
    ...
    while (!q.empty())
    {
        ...
        if(temp_r >= 0){
            if(mat[temp_c+1][temp_r]) ans[temp_c][temp_r+1] += ans[temp_c][temp_r] + 1;
            if(visited[temp_c][temp_r+1]) continue;
            q.push(make_pair(temp_c, temp_r+1));
            visited[temp_c][temp_r+1] = 2;
        } 
        if(temp_r <= mat[0].size()){
            if(mat[temp_c+1][temp_r]) ans[temp_c+1][temp_r-1] += ans[temp_c][temp_r] + 1;
            if(visited[temp_c][temp_r-1]) continue;
            q.push(make_pair(temp_c, temp_r-1));
            visited[temp_c][temp_r-1] = 2;
        }
