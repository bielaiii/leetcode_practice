...
int maxCherry = 0;
void dfs_cherryPickup(const vector<vector<int>>& grid, int x, int y, vector<vector<bool>>& visited, int &guide, int terminal, int current_cherry){
    ...
    for(int j = 0; j < 2; j++){
        ...
        if(cur_x > -1 && x + cur_x < grid.size() &&
           cur_y > -1 && x + cur_y < grid.size() &&
           grid[cur_x][cur_y] != -1 && 
           !visited[cur_x][cur_y]
           ){
            visited[cur_x][cur_y ]= 1;
            if(grid[cur_x][cur_y] == 1) current_cherry ++;
