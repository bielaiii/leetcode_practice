...
int cutOffTree(vector<vector<int>>& forest) {
    ...
    for(int i = 0; i < forest.size(); i++){
        for(int j = 0; j < forest[0].size(); j++){
            ...
            if(forest[i][j] > 1){
                tree ++;;
                set_.insert(i*forest[0].size() + j);
            } 
        }
    }
    sort(node_.begin(), node_.end(), )
    int step = 0;
    int x = 0, y = 0;
    int next_x = 0, next_y = 0 ;
    pair<int,int> start = {0, 0};
    for(auto & it : set_){
       // if(it + 1 == set_.end()) break;;
       int next_x = (it + 1) % forest[0].size();
       int next_y = (it + 1) / forest[0].size();
       pair<int,int> target_ = {next_x, next_y}; 
        int ret =  bfs(forest, start, target_);
        if(ret ) tree --;
        start = target_;
        step += ret;
    }
