...
TreeNode* recur_pruneTree(TreeNode *root){
    ...
    if(root->val == 1){
        ...
        root = recur_pruneTree(root->left);
        root = recur_pruneTree(root->right);
    }
    return root;
}
...
void traversal_pruneTree(TreeNode * root){
    ...
    if(root->val == 0){
        ...
        if(root->left != nullptr) traversal_pruneTree(root->left);
        if(root->right != nullptr) traversal_pruneTree(root->right);
    }else{
