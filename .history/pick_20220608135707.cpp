...
public:
    ...
    vector<int> pick() {
        ...
        while(temp_area > 0 && left < this->areas.size()){
            temp_area -= this->areas[left];
            left++;
        }
        ...
        uniform_int_distribution<int> temp_x(all_rects[left][0], all_rects[left][2]);            
        uniform_int_distribution<int> temp_y(all_rects[left][1], all_rects[left][3]);    
